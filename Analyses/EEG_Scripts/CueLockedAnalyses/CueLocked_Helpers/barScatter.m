...
% Adapted by Johannes Algermissen to include color coding option and x-axis position for bars
...
if bScatter
    for t = 1:nbars
        ...
        s = scatter(xpos(t)*ones(1,length(tmp)),tmp,[],'k', 'jitter','on', 'jitterAmount',0.05);
        set(s,'MarkerEdgeColor',[0.4 0.4 0.4],'linewidth',2);
    end
end
th=errorbar(xpos,barvalues, errors, 'r', 'linestyle', 'none','linewidth',2.5);
set(th,'Linewidth',4,'color',[0 0 0]);
he=[he;th];
