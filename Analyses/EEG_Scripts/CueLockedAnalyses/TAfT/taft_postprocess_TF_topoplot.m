...
for iFreq = 1:length(startFreq) % % loop over given frequency ranges
    ...
    for iPlot = 1:length(endTimeVec) % loop over given time windows
        ...
        cfg = []; cfg.figure = gcf; cfg.ylim = selFreqs; cfg.zlim = [-1*zlim 1*zlim]; cfg.marker='on'; cfg.style='straight';
    ...
    sgtitle(sprintf('Topoplot for ROI %s, %s-locked, frequencies %d - %d Hz, time window %.1f - %.1f sec.',...
        char(job.regNames(iROI)),job.lockName,selFreqs(1),selFreqs(end),startTimeVec(1),endTimeVec(end)),'fontsize',fontSize)
    ...
    if isSave
       saveas(gcf,fullfile(dirs.topoplot,sprintf('Topoplot_TF_%s_%s_%s_%dsec_%d-%dHz_%s-%sms.png',...
           char(job.regNames(iROI)),job.lockSettings,job.HRFtype,job.trialdur,...
           selFreqs(1),selFreqs(end),num2str(1000*startTimeVec(1)),num2str(1000*endTimeVec(end)))))
    end
