...
% Like GLM1, but split up per accuracy (correct/ incorrect).
% Allows for contrasts valence, performed action, conflict 
% accuracy (8 regressors).
...
% (Go/NoGo):
% 1) Win_ActGo_Correct
% 2) Win_ActGo_Incorrect
% 3) Win_ActNoGo_Correct
% 4) Win_ActNoGo_Incorrect
% 5) Avoid_ActGo_Correct
% 6) Avoid_ActGo_Incorrect
% 7) Avoid_ActNoGo_Correct
% 8) Avoid_ActNoGo_Incorrect
% 
...
GLMID       = '2';
nRegressors = 12; % for specification of empty regressors
...
for iSub = 1:nSub % length(dirs.subs) % specify for which subjects to create regressors
    ...
    Win_ActGo_Correct_Trials        = (ismember(stimIDAll,[1 2 5 6]) & ismember(respNumAll,[1 2]) & correctAll==1 & cueDetectAll==1); % indicator whether trial was Win trial with Go response, correct
    Win_ActGo_Incorrect_Trials      = (ismember(stimIDAll,[1 2 5 6]) & ismember(respNumAll,[1 2]) & correctAll==0 & cueDetectAll==1); % indicator whether trial was Win trial with Go response, incorrect
    Win_ActNoGo_Correct_Trials      = (ismember(stimIDAll,[1 2 5 6]) & ismember(respNumAll,[3]) & correctAll==1 & cueDetectAll==1); % indicator whether trial was Win trial with Go response, correct
    Win_ActNoGo_Incorrect_Trials    = (ismember(stimIDAll,[1 2 5 6]) & ismember(respNumAll,[3]) & correctAll==0 & cueDetectAll==1); % indicator whether trial was Win trial with Go response, incorrect
    Avoid_ActGo_Correct_Trials      = (ismember(stimIDAll,[3 4 7 8]) & ismember(respNumAll,[1 2]) & correctAll==1 & cueDetectAll==1); % indicator whether trial was Win trial with Go response, correct
    Avoid_ActGo_Incorrect_Trials    = (ismember(stimIDAll,[3 4 7 8]) & ismember(respNumAll,[1 2]) & correctAll==0 & cueDetectAll==1); % indicator whether trial was Win trial with Go response, incorrect
    Avoid_ActNoGo_Correct_Trials    = (ismember(stimIDAll,[3 4 7 8]) & ismember(respNumAll,[3]) & correctAll==1 & cueDetectAll==1); % indicator whether trial was Win trial with Go response, correct
    Avoid_ActNoGo_Incorrect_Trials  = (ismember(stimIDAll,[3 4 7 8]) & ismember(respNumAll,[3]) & correctAll==0 & cueDetectAll==1); % indicator whether trial was Win trial with Go response, incorrect

    % Handedness: motorNumAll craeted above
    ...
    % 1) Win ActGo Correct response:
    if sum(Win_ActGo_Correct_Trials) == 0
        tWin_ActGo_Correct = [0 0 0];
        emptyRegressorMatrix(1) = 1; % specify that regressor is empty (regressor position hard-coded given that GLM setup is known)
        fprintf('Note: Subject %d has empty regressors tWin_ActGo_Correct! \n', iSub)
    else
        tWin_ActGo_Correct  = [tCueAll(Win_ActGo_Correct_Trials ==1) ones(sum(Win_ActGo_Correct_Trials),2)];
    end
    
   % 2) Win ActGo Incorrect response:
    if sum(Win_ActGo_Incorrect_Trials) == 0
        tWin_ActGo_Incorrect = [0 0 0];
        emptyRegressorMatrix(2) = 1; % specify that regressor is empty (regressor position hard-coded given that GLM setup is known)
        fprintf('Note: Subject %d has empty regressors tWin_ActGo_Incorrect! \n', iSub)
    else
        tWin_ActGo_Incorrect  = [tCueAll(Win_ActGo_Incorrect_Trials ==1) ones(sum(Win_ActGo_Incorrect_Trials),2)];
    end
    
    % 3) Win ActNoGo Correct response:
    if sum(Win_ActNoGo_Correct_Trials) == 0
        tWin_ActNoGo_Correct = [0 0 0];
        emptyRegressorMatrix(3) = 1; % specify that regressor is empty (regressor position hard-coded given that GLM setup is known)
        fprintf('Note: Subject %d has empty regressors tWin_ActNoGo_Correct! \n', iSub)
    else
        tWin_ActNoGo_Correct = [tCueAll(Win_ActNoGo_Correct_Trials ==1) ones(sum(Win_ActNoGo_Correct_Trials),2)];
    end
    
    % 4) Win ActNoGo Correct response:
    if sum(Win_ActNoGo_Incorrect_Trials) == 0
        tWin_ActNoGo_Incorrect = [0 0 0];
        emptyRegressorMatrix(4) = 1; % specify that regressor is empty (regressor position hard-coded given that GLM setup is known)
        fprintf('Note: Subject %d has empty regressors tWin_ActNoGo_Incorrect! \n', iSub)
    else
        tWin_ActNoGo_Incorrect = [tCueAll(Win_ActNoGo_Incorrect_Trials ==1) ones(sum(Win_ActNoGo_Incorrect_Trials),2)];
    end
    
    % 5) Avoid ActGo Correct response:
    if sum(Avoid_ActGo_Correct_Trials) == 0
        tAvoid_ActGo_Correct = [0 0 0];
        emptyRegressorMatrix(5) = 1; % specify that regressor is empty (regressor position hard-coded given that GLM setup is known)
        fprintf('Note: Subject %d has empty regressors tAvoid_ActGo_Correct! \n', iSub)
    else
        tAvoid_ActGo_Correct  = [tCueAll(Avoid_ActGo_Correct_Trials ==1) ones(sum(Avoid_ActGo_Correct_Trials),2)];
    end
    
    % 6) Avoid ActGo Correct response:
    if sum(Avoid_ActGo_Incorrect_Trials) == 0
        tAvoid_ActGo_Incorrect = [0 0 0];
        emptyRegressorMatrix(6) = 1; % specify that regressor is empty (regressor position hard-coded given that GLM setup is known)
        fprintf('Note: Subject %d has empty regressors tAvoid_ActGo_Incorrect! \n', iSub)
    else
        tAvoid_ActGo_Incorrect  = [tCueAll(Avoid_ActGo_Incorrect_Trials ==1) ones(sum(Avoid_ActGo_Incorrect_Trials),2)];
    end
    
    % 7) Avoid ActNoGo Correct response:
    if sum(Avoid_ActNoGo_Correct_Trials) == 0
        tAvoid_ActNoGo_Correct = [0 0 0];
        emptyRegressorMatrix(7) = 1; % specify that regressor is empty (regressor position hard-coded given that GLM setup is known)
        fprintf('Note: Subject %d has empty regressors tAvoid_ActNoGo_Correct! \n', iSub)
    else
        tAvoid_ActNoGo_Correct = [tCueAll(Avoid_ActNoGo_Correct_Trials ==1) ones(sum(Avoid_ActNoGo_Correct_Trials),2)];
    end
    
    % 8) Avoid ActNoGo Correct response:
    if sum(Avoid_ActNoGo_Incorrect_Trials) == 0
        tAvoid_ActNoGo_Incorrect = [0 0 0];
        emptyRegressorMatrix(8) = 1; % specify that regressor is empty (regressor position hard-coded given that GLM setup is known)
        fprintf('Note: Subject %d has empty regressors tAvoid_ActNoGo_Incorrect! \n', iSub)
    else
        tAvoid_ActNoGo_Incorrect = [tCueAll(Avoid_ActNoGo_Incorrect_Trials ==1) ones(sum(Avoid_ActNoGo_Incorrect_Trials),2)];
    end

    % Check for NAs:
    if sum(isnan(tWin_ActGo_Correct(:,3))) > 0; fprintf("Sub%d tWin_ActGo_Correct has NAs!",iSub); end
    if sum(isnan(tWin_ActGo_Incorrect(:,3))) > 0; fprintf("Sub%d tWin_ActGo_Incorrect has NAs!",iSub); end
    if sum(isnan(tWin_ActNoGo_Correct(:,3))) > 0; fprintf("Sub%d tWin_ActNoGo_Correct has NAs!",iSub); end
    if sum(isnan(tWin_ActNoGo_Incorrect(:,3))) > 0; fprintf("Sub%d tWin_ActNoGo_Incorrect has NAs!",iSub); end
    if sum(isnan(tAvoid_ActGo_Correct(:,3))) > 0; fprintf("Sub%d tAvoid_ActGo_Correct has NAs!",iSub); end
    if sum(isnan(tAvoid_ActGo_Incorrect(:,3))) > 0; fprintf("Sub%d tAvoid_ActGo_Incorrect has NAs!",iSub); end
    if sum(isnan(tAvoid_ActNoGo_Correct(:,3))) > 0; fprintf("Sub%d tAvoid_ActNoGo_Correct has NAs!",iSub); end
    if sum(isnan(tAvoid_ActNoGo_Incorrect(:,3))) > 0; fprintf("Sub%d tAvoid_ActNoGo_Incorrect has NAs!",iSub); end
    ...
    % 10-11) Outcome onset: neutral, reward after Go/NoGo, punishment after Go/NoGo.
    ...
    % 12) Invalid trials. Uninstructed key pressed, thus no outcome, but error message.
    if sum(InvalidTrials) == 0 % if in fact no uninstructed key presses
        tInvalid = [0 0 0];
        fprintf('Note: Subject %d has empty regressor Invalid! \n', iSub)
        emptyRegressorMatrix(12) = 1; % specify that regressor is empty (regressor position hard-coded given that GLM setup is known)
    else
    ...
    % Onsets:
    save(fullfile(dirs.timings,'tWin_ActGo_Correct.txt'),'tWin_ActGo_Correct','-ascii')
    save(fullfile(dirs.timings,'tWin_ActGo_Incorrect.txt'),'tWin_ActGo_Incorrect','-ascii')
    save(fullfile(dirs.timings,'tWin_ActNoGo_Correct.txt'),'tWin_ActNoGo_Correct','-ascii')
    save(fullfile(dirs.timings,'tWin_ActNoGo_Incorrect.txt'),'tWin_ActNoGo_Incorrect','-ascii')
    save(fullfile(dirs.timings,'tAvoid_ActGo_Correct.txt'),'tAvoid_ActGo_Correct','-ascii')
    save(fullfile(dirs.timings,'tAvoid_ActGo_Incorrect.txt'),'tAvoid_ActGo_Incorrect','-ascii')
    save(fullfile(dirs.timings,'tAvoid_ActNoGo_Correct.txt'),'tAvoid_ActNoGo_Correct','-ascii')
    save(fullfile(dirs.timings,'tAvoid_ActNoGo_Incorrect.txt'),'tAvoid_ActNoGo_Incorrect','-ascii')
...
% Subject 010: Start 
% Subject 011: Start 
% Note: Subject 11 has empty regressors tWin_ActNoGo_Incorrect! 
% Subject 012: Start 
...
% Subject 024: Start 
% Note: Subject 24 has empty regressors tWin_ActNoGo_Incorrect! 
